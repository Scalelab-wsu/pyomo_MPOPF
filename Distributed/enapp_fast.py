...
def process_area(data_areas,area_name,obj_fcn,prev_solution = None):
    global _model_cache
...
def exclude_dummies(dopfVals):
    ...
    for var in dopfVals.keys():
        # Only filter for 'P', 'Q', and 'v'
        if var in ['P', 'Q', 'v']:
            filtered_dictionaries[var] = {}  # Initialize an empty dictionary for each of these vars
            for key, value in dopfVals[var].items():
                key1 = key[1]  # Get the second element of the key
                # Skip the entry if key[1] is a tuple and contains any strings
                if isinstance(key1, tuple) and any(isinstance(sub, str) for sub in key1):
                    continue
                # Also skip the entry if key[1] is a string directly
                if isinstance(key1, str):
                    continue
                # Otherwise, add it to the filtered dictionary
                filtered_dictionaries[var][key] = value
        else:
            # For all other variables, simply copy them over
            filtered_dictionaries[var] = dopfVals[var]
    ...
    with mp.Pool(processes=len(areas), initializer=init_worker) as pool:
        for iter in range(max_iterations):
            ...
            results = pool.starmap(process_area,
                                   [(data_by_area[area], area, obj_fcn)
                                    for area in areas])

...
def remove_inter_area_edges(g, area_info,full_data):
            ...
            if g.has_edge(fb, tb):
                ...
                for key in dummy_keys:
                    ...
                    if (fb, tb) in full_data['Lset']:
                        ...
                        print(
                            f"Warning: Impedance data for edge ({fb}, {tb}) not found in 'r[{key}]' or 'x[{key}]'. Assigning 0.")
                        dummy_r_value = 0
                        dummy_x_value = 0
                    # Assign to new edges
    ...
    return g,dummy_edge_r,dummy_edge_x


###########################
# 6) Build sub-area data
###########################
def build_area_data(full_data, area_name, sg_local, dummy_r,dummy_x):
    ...
    p_B_area = {
        ...
        for key in full_data["p_B"]
        if key[0] in Bset_area
    }
    s_B_area = {
        key: full_data["s_B"][key]
        for key in full_data["s_B"]
        if key[0] in Bset_area
    }
    eta_c_area = {
    ...
    bmin_area = {
        ...
        for key in full_data["bmin"]
        if key[0] in Bset_area
    }
    bmax_area = {
        key: full_data["bmax"][key]
        for key in full_data["bmax"]
        if key[0] in Bset_area
    }
    b0_area = {
        key: full_data["b0"][key]
        for key in full_data["b0"]
        if key[0] in Bset_area
    }
...
def split_data_into_areas(full_data,area_info):
    ...
    # ----------------------------
    g,dummy_r,dummy_x = remove_inter_area_edges(g, area_info,full_data)
    ...
    for area_name, sg_local in area_subgraphs.items():
        data_area = build_area_data(full_data, area_name, sg_local,dummy_r,dummy_x)

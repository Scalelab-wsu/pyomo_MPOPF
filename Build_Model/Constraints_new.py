...
def build_pyomo_model(data):
    ...
    model.p_D = data['p_D']
    model.T = data['T']
    model.p_L = Param(model.Tset, model.Nset, model.phases,initialize=data['p_L'],mutable=True)
    model.q_L = Param(model.Tset, model.Nset, model.phases, initialize=data['q_L'], mutable=True)
    model.v_swing = Param(model.Tset, model.substationBus, model.phases, initialize=data['v_swing'], mutable=True)
    ...
    def battery_dynamics_rule(model, t, j, ph):
        ...
        if n_d == 0:
            n_d = math.inf
        if t == 1:
            return model.B[t, j, ph] == b0 + (model.P_c[t, j, ph] * n_c) - (model.P_d[t, j, ph] / n_d)
        else:
            return model.B[t, j, ph] == model.B[t - 1, j, ph] + (model.P_c[t, j, ph] * n_c) - (model.P_d[t, j, ph] / n_d)

...
def build_pyomo_model(data, obj, stage_idx = None, non_linear=False, isocp=False,p_control=False, integer=False,single_battery_variable=False):
    ...
    model.branch_phase_pair_set = Set(initialize=[((i, j), p, q) for (i, j) in model.Lset for p in branch_phases[(i, j)] for q in branch_phases[(i, j)]if p<=q])
    ...
    def real_power_balance_rule(model, t, j, ph):
        ...
        if non_linear or isocp:
            ...
            loss_term = 0
            if ph == 'a':
                loss_term += sum((r['aa'][i, jj]* model.l[t, i, jj, 'a', 'a'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'b' in b_phases:
                    loss_term += sum((r['ab'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']) + x['ab'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']))* model.l[t, i, jj, 'a', 'b'] for i, jj in model.Lset if jj == j)
                if 'c' in b_phases:
                    loss_term += sum((r['ac'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']) + x['ac'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'a', 'c'] for i, jj in model.Lset if jj == j)
            elif ph == 'b':
                loss_term += sum((r['bb'][i, jj]* model.l[t, i, jj, 'b', 'b'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'a' in b_phases:
                    loss_term += sum((r['ab'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']) - x['ab'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']))* model.l[t, i, jj, 'a', 'b'] for i, jj in model.Lset if jj == j)
                if 'c' in b_phases:
                    loss_term += sum((r['bc'][i,jj]*cos(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']) + x['bc'][i,jj]*sin(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'b', 'c'] for i, jj in model.Lset if jj == j)
            elif ph == 'c':
                loss_term += sum((r['cc'][i, jj]* model.l[t, i, jj, 'c', 'c'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'a' in b_phases:
                    loss_term += sum((r['ac'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']) - x['ac'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'a', 'c'] for i, jj in model.Lset if jj == j)
                if 'b' in b_phases:
                    loss_term += sum((r['bc'][i,jj]*cos(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']) - x['bc'][i,jj]*sin(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'b', 'c'] for i, jj in model.Lset if jj == j)
    ...
    def reactive_power_balance_rule(model, t, j, ph):
        ...
        if non_linear or isocp:
            ...
            loss_term = 0
            if ph == 'a':
                loss_term += sum((x['aa'][i, jj]* model.l[t, i, jj, 'a', 'a'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'b' in b_phases:
                    loss_term += sum((x['ab'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']) - r['ab'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']))* model.l[t, i, jj, 'a', 'b'] for i, jj in model.Lset if jj == j)
                if 'c' in b_phases:
                    loss_term += sum((x['ac'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']) - r['ac'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'a', 'c'] for i, jj in model.Lset if jj == j)
            elif ph == 'b':
                loss_term += sum((x['bb'][i, jj]* model.l[t, i, jj, 'b', 'b'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'a' in b_phases:
                    loss_term += sum((x['ab'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']) + r['ab'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'b']))* model.l[t, i, jj, 'a', 'b'] for i, jj in model.Lset if jj == j)
                if 'c' in b_phases:
                    loss_term += sum((x['bc'][i,jj]*cos(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']) - r['bc'][i,jj]*sin(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'b', 'c'] for i, jj in model.Lset if jj == j)
            elif ph == 'c':
                loss_term += sum((x['cc'][i, jj]* model.l[t, i, jj, 'c', 'c'] for i, jj in model.Lset if jj == j and ph in branch_phases[(i, jj)]))
                if 'a' in b_phases:
                    loss_term += sum((x['ac'][i,jj]*cos(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']) + r['ac'][i,jj]*sin(model.delta[t, i, jj, 'a'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'a', 'c'] for i, jj in model.Lset if jj == j)
                if 'b' in b_phases:
                    loss_term += sum((x['bc'][i,jj]*cos(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']) + r['bc'][i,jj]*sin(model.delta[t, i, jj, 'b'] - model.delta[t, i, jj, 'c']))* model.l[t, i, jj, 'b', 'c'] for i, jj in model.Lset if jj == j)
    ...
    def kvl_three_phase_rule(model, t, i, j, ph):
        ...
        if non_linear or isocp:
            loss_term = 0
            if ph == 'a':
                loss_term += (r['aa'][i, j]**2 + x['aa'][i, j]**2)*model.l[t,i,j,'a','a']
                if 'b' in br_phases:
                    loss_term += (r['ab'][i, j] ** 2 + x['ab'][i, j] ** 2) * model.l[t, i, j, 'b', 'b'] + 2*((r['aa'][i, j]*r['ab'][i, j]+x['aa'][i, j]*x['ab'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b']))-(x['aa'][i, j]*r['ab'][i, j]-r['aa'][i, j]*x['ab'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b'])))*model.l[t,i,j,'a', 'b']
                if 'c' in br_phases:
                    loss_term += (r['ac'][i, j] ** 2 + x['ac'][i, j] ** 2) * model.l[t, i, j, 'c', 'c']+ 2*((r['aa'][i, j]*r['ac'][i, j]+x['aa'][i, j]*x['ac'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c']))-(x['aa'][i, j]*r['ac'][i, j]-r['aa'][i, j]*x['ac'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'a', 'c']
                if 'b' in br_phases and 'c' in br_phases:
                    loss_term += 2*((r['ab'][i, j]*r['ac'][i, j]+x['ab'][i, j]*x['ac'][i, j])*cos(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c']))-(x['ab'][i, j]*r['ac'][i, j]-r['ab'][i, j]*x['ac'][i, j])*sin(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'b', 'c']
            if ph == 'b':
                loss_term += (r['bb'][i, j]**2 + x['bb'][i, j]**2)*model.l[t,i,j,'b','b']
                if 'a' in br_phases:
                    loss_term += (r['ab'][i, j] ** 2 + x['ab'][i, j] ** 2) * model.l[t, i, j, 'a', 'a'] + 2*((r['bb'][i, j]*r['ab'][i, j]+x['bb'][i, j]*x['ab'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b']))-(r['bb'][i, j]*x['ab'][i, j]-x['bb'][i, j]*r['ab'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b'])))*model.l[t,i,j,'a', 'b']
                if 'c' in br_phases:
                    loss_term += (r['bc'][i, j] ** 2 + x['bc'][i, j] ** 2) * model.l[t, i, j, 'c', 'c']+ 2*((r['bb'][i, j]*r['bc'][i, j]+x['bb'][i, j]*x['bc'][i, j])*cos(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c']))-(r['bc'][i, j]*x['bb'][i, j]-x['bc'][i, j]*r['bb'][i, j])*sin(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'b', 'c']
                if 'a' in br_phases and 'c' in br_phases:
                    loss_term += 2*((r['ab'][i, j]*r['bc'][i, j]+x['ab'][i, j]*x['bc'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c']))-(x['ab'][i, j]*r['bc'][i, j]-r['ab'][i, j]*x['bc'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'a', 'c']
            if ph == 'c':
                loss_term += (r['cc'][i, j]**2 + x['cc'][i, j]**2)*model.l[t,i,j,'c','c']
                if 'a' in br_phases:
                    loss_term += (r['ac'][i, j]** 2 + x['ac'][i, j] ** 2) * model.l[t, i, j, 'a', 'a'] + 2*((r['cc'][i, j]*r['ac'][i, j]+x['cc'][i, j]*x['ac'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c']))-(r['cc'][i, j]*x['ac'][i, j]-x['cc'][i, j]*r['ac'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'a', 'c']
                if 'b' in br_phases:
                    loss_term += (r['bc'][i, j] ** 2 + x['bc'][i, j] ** 2) * model.l[t, i, j, 'b', 'b']+ 2*((r['bc'][i, j]*r['cc'][i, j]+x['bc'][i, j]*x['cc'][i, j])*cos(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c']))-(r['cc'][i, j]*x['bc'][i, j]-x['cc'][i, j]*r['bc'][i, j])*sin(model.delta[t,i,j,'b']-(model.delta[t,i,j,'c'])))*model.l[t,i,j,'b', 'c']
                if 'a' in br_phases and 'b' in br_phases:
                    loss_term += 2*((r['bc'][i, j]*r['ac'][i, j]+x['bc'][i, j]*x['ac'][i, j])*cos(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b']))-(x['ac'][i, j]*r['bc'][i, j]-r['ac'][i, j]*x['bc'][i, j])*sin(model.delta[t,i,j,'a']-(model.delta[t,i,j,'b'])))*model.l[t,i,j,'a', 'b']
        else:
            loss_term = 0

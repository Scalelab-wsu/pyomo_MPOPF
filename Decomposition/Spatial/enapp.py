...
def process_area(data_areas, area_name, obj_fcn, prev_solution=None, non_linear=False, p_control=False, integer=False):
    global _model_cache
...
def solve_EnAPP(data, data_by_area, area_info, obj_fcn, *, max_iterations=50, alpha=0, non_linear=False, p_control=False, integer=False):
    ...
    with mp.Pool(processes=len(areas), initializer=init_worker) as pool:
        for iter in range(max_iterations):
            ...
            results = pool.starmap(
                process_area,
                [
                    (data_by_area[area], area, obj_fcn, None, non_linear, p_control, integer)
                    for area in areas
                ],
            )

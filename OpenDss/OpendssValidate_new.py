...
def collect_opendss_substationpower(openDssVals,t,P_base):
    ...
    while line_id == 1:
        ...
        ph_letters = [{'1': 'a', '2': 'b', '3': 'c'}[str(n)] for n in ph_number]
...
def collect_opendss_lineflows(openDssVals,t,P_base):
    ...
    while line_id > 0:
        ...
        ph_number = busnames[0].split('.')[1:]
        ph_letters = [{'1': 'a', '2': 'b', '3': 'c'}[str(n)] for n in ph_number]
        fb = int(busnames[0].split('.')[0])
        tb =  int(busnames[1].split('.')[0])
        for ph_idx, ph in enumerate(ph_letters):
            p = line_powers[2 * ph_idx] * 1000 / P_base
            q = line_powers[2 * ph_idx + 1] * 1000 / P_base
...
def collect_opendss_pvPowers(openDssVals,t, P_base):
    ...
    while pv_id > 0:
        ...
        bus = int(bus_ph[0])
        ph_nodes = bus_ph[1:]  # e.g., ["1", "2"] for phases a, b
        for ph_node in ph_nodes:
            ph = {'1': 'a', '2': 'b', '3': 'c'}[ph_node]
...
def collect_opendss_batteryresults(openDssVals,t, P_base):
    ...
    while bat_id > 0:
        ...
        ph = {'1': 'a', '2': 'b', '3': 'c'}[ph_node]
        p = -dss.CktElement.Powers()[0] * 1000 / P_base
        soc = dss.Storages.puSOC()*0.01 ## multiplying by rated to match optimization results for comparison
        if p >= 0:
            openDssVals['P_d'][(t, bus, ph)] = p
...
def run_opendss_validation(data, modelVals):
    ...
    for t in data['Tset']:
        ...
        openDssVals = collect_opendss_batteryresults(openDssVals, t, P_base)
    ...
    print(f"total PV KVAR : {total_pv_kvar}")
...
def all_time_highest_discrepancy(opendss,optimization):
    ...
    overall_max_v = max(np.max(np.abs(V_vals_opendss[key] - np.sqrt(V_vals_optimization[key]))) for key in V_vals_opendss.keys())

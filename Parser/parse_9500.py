...
def parse_all_data(bus, branch, gen=None, bat=None, loadshape=None, pvshape=None, price=None,start_step = 1, n_steps = 24):
    ...
    v_base = {i: bus_lookup.at[i, "v_ln_base"] for i in bus_set}

    ## parsing branch_data
    branch = branch.loc[branch.status != 'OPEN']
    branch_set = sorted(set(zip(branch['fb'], branch['tb'])))
    lines_only = branch.loc[~branch['type'].isin(['reactor', 'transformer'])]
    lines_only_set = sorted(set(zip(lines_only['fb'], lines_only['tb'])))
    ...
    z_base = {"z":csr_matrix((np.r_[branch.z_base, branch.z_base], (row, col)))}
